...
void mainMenu()
{
    ...
    do
    {
        ...
        {
        case 1:
            studentManagementMenu(); // Hàm menu quản lý sinh viên
            break;
        case 2:
            teacherManagementMenu(); // Hàm menu quản lý giáo viên
            break;
        case 3:
            classManagementMenu(); // Hàm menu quản lý lớp
            break;
        case 0:
...
void teacherManagementMenu()
{
    ...
    do
    {
        ...
        {
        case 1:
            showTeacherList(); // Hàm hiển thị danh sách giáo viên
            break;
        case 2:
            addNewTeacher(); // Hàm thêm mới giáo viên
            break;
        case 3:
            editTeacherInfo(); // Hàm chỉnh sửa thông tin giáo viên
            break;
        case 4:
            deleteTeacher(); // Hàm xóa giáo viên
            break;
        case 5:
            searchTeacher(); // Hàm tìm kiếm giáo viên
            break;
        case 6:
            addTeacherToClass(); // Hàm thêm giáo viên vào lớp học
            break;
        case 0:
            printf("\n\t\t-----Quay lai Menu chinh-----\n");
            break;
...
void classManagementMenu()
{
    ...
    do
    {
        ...
        {
        case 1:
            showClassList(); // Hàm hiển thị danh sách lớp học
            break;
        case 2:
            addNewClass(); // Hàm thêm mới lớp học
            break;
        case 3:
            editClassInfo(); // Hàm sửa thông tin lớp học
            break;
        case 4:
            deleteClass(); // Hàm xóa lớp học
            break;
        case 5:
            classDetailMenu(); // Hàm menu nhỏ của chức năng quản lý lớp học
            break;
        case 6:
            sortClassListMenu(); // Hàm menu lựa chọn thứ tự sắp xếp lớp học
            break;
        case 0:
            printf("\n\t\t-----Quay lai Menu chinh-----\n");
            break;
...
void classDetailMenu()
{
    ...
    do
    {
        ...
        {
        case 1:
            showClassDetail(); // Hàm hiển thị thông tin chi tiết của lớp
            break;
        case 2:
            addStudentToClass(); //  Hàm thêm sinh viên vào lớp học
            break;
        case 3:
            deleteStudentFromClass(); // Hàm xóa sinh viên khỏi lớp học
            break;
        case 0:
...
int checkDuplicateStudentInfo(char *IDToCheck, char *emailToCheck, char *phoneNumberToCheck)
{
    ...
    {
        if (strcmp(existingStudent.studentID, IDToCheck) == 0)
        {
...
int checkDuplicateTeacherInfo(char *IDToCheck, char *emailToCheck, char *phoneNumberToCheck)
{
    ...
    {
        if (strcmp(existingTeacher.teacherID, IDToCheck) == 0)
        {
...
int checkDuplicateClassInfo(char *IDToCheck, char *nameToCheck)
{
    ...
    {
        if (strcmp(existingClass.classID, IDToCheck) == 0)
        {
...
// Hàm thêm giáo viên vào lớp học
...
void editClassInfo()
{
    ...
    while (fread(&editClassInfo, sizeof(struct Class), 1, file))
    {
        // So sánh chuỗi ID lớp học
        if (strcmp(editClassInfo.classID, IDToSearch) == 0)
        {
            classFound = 1;
            printf("Tim thay lop hoc: %s\n", editClassInfo.classID);

            // Cập nhật thông tin lớp học
            printf("Nhap ten moi cua lop: ");
            getchar(); // Xóa ký tự xuống dòng còn lại
            fgets(editClassInfo.className, sizeof(editClassInfo.className), stdin);
            editClassInfo.className[strcspn(editClassInfo.className, "\n")] = '\0';

            printf("Nhap nganh hoc moi: ");
            fgets(editClassInfo.classMajor, sizeof(editClassInfo.classMajor), stdin);
            editClassInfo.classMajor[strcspn(editClassInfo.classMajor, "\n")] = '\0';

            printf("Nhap trang thai moi (0: Khong hoat dong, 1: Hoat dong): ");
            scanf("%d", &editClassInfo.classStatus);

            if (!checkDuplicateClassInfo(editClassInfo.classID, editClassInfo.className))
            {
                ...
                if (fwrite(&editClassInfo, sizeof(struct Class), 1, file) == 1)
                {
                    printf("\n\t=========> Cap nhat thong tin hoan tat <=========\n");
                }
                else
...
void showClassDetail()
{
    ...
    if (studentCount == 0)
    {
        printf("\n\t\t\t\t\t\t\t\t\t=========> Lop hoc nay chua co sinh vien nao <=========\n");
    }
...
// Hàm ẩn mật khẩu
void inputHiddenPassword(char *input, int maxLength)
{
    int index = 0;
    char inputChar;
    ...
    while (1)
    {
        inputChar = getch(); // Nhấn enter
        if (inputChar == 13)
        {
            input[index] = '\0';
            break;
        }
        else if (inputChar == 8) // Nhấn backspace
        {
            if (index > 0)
            {
                index--;

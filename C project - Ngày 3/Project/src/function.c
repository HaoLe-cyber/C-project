...
void addNewStudent()
{
    ...
    if (checkDuplicateInfo(newstudentInfo.studentEmail, newstudentInfo.studentPhoneNumber))
    {
...
void editStudentInfo()
{
    ...
    while (fread(&editStudentInfo, sizeof(struct Student), 1, inputFile))
    {
        if (strcmp(editStudentInfo.studentID, IDToSearch) == 0)
        {
            ...
            if (!checkDuplicateInfo(editStudentInfo.studentEmail, editStudentInfo.studentPhoneNumber))
            {
...
void addNewTeacher()
{
    ...
    if (checkDuplicateInfo(newTeacherInfo.teacherEmail, newTeacherInfo.teacherPhoneNumber))
    {
    ...
    {
        printf("\n\t=========> Them giao vien that bai (du lieu khong hop le) <=========\n\n");
    }
    fclose(inputFile);
};
...
void editTeacherInfo()
{
    ...
    while (fread(&editTeacherInfo, sizeof(struct Teacher), 1, inputFile))
    {
        if (strcmp(editTeacherInfo.teacherID, IDToSearch) == 0)
        {
            ...
            if (!checkDuplicateInfo(editTeacherInfo.teacherEmail, editTeacherInfo.teacherPhoneNumber))
            {
                printf("\n\t=========> Chinh sua giao vien that bai do thong tin khong hop le <=========\n");
            }
            else
            {
...
void addNewClass()
{
    ...
    inputFile = fopen("D:\\Code C task\\My Main Project\\Project\\data\\Class_Data.bin", "ab");
    if (inputFile == NULL)
    {
...
void editClassInfo()
{
    ...
    strcpy(editClassInfo.classTeacherName, ""); // Gán giá trị ban đầu cho classTeacherName để đảm bảo không bị chèn dữ liệu rác
    ...
    while (fread(&editClassInfo, sizeof(struct Class), 1, inputFile))
    {
        if (strcmp(editClassInfo.classID, IDToSearch) == 0)
        {
            ...
            } while (!isValidName(editClassInfo.className));
            ...
            printf("Nhap trang thai moi (0: Khong giang day, 1: Dang giang day): ");
            scanf("%d", &editClassInfo.classStatus);

            // Ghi đè thông tin lớp học
            fseek(inputFile, -sizeof(struct Class), SEEK_CUR);
            if (fwrite(&editClassInfo, sizeof(struct Class), 1, inputFile) == 1)
            {
                printf("\n\t=========> Cap nhat thong tin hoan tat <=========\n");
            }
            else
